...
from domino.data.celeb import ATTRIBUTES
...
def get_model_predictions(
    ...
    bs_num_iter:int=100, run_dir=True
):
...
def compute_celeb_subgroup_perf(
    ...
    for target_attr in tqdm(ATTRIBUTES):  
        ci = auroc_bootstrap_ci(celeb_dp[target_attr].data,  celeb_dp[f"prob_{target_attr}"].data[:, -1], num_iter=bs_num_iter)
        results.append({
            **ci, 
        ...
        for slice_attr in ATTRIBUTES:
            ...
            for slice_value in [0,1]:
                ...
                if len(np.unique(curr_dp[target_attr])) != 2:
                    continue
                ci = auroc_bootstrap_ci(curr_dp[target_attr].data,  curr_dp[f"prob_{target_attr}"].data[:, -1], num_iter=bs_num_iter)
                results.append({
                    **ci, 
                    "target_attr": target_attr,
