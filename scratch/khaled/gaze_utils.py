...
def generate_ideal_observer(
    ...
    while np.max(posterior) < threshold and len(gaze_seq) < max_nfix:
        curr_loc = gaze_seq[-1]
        d_map = calc_dmap(curr_loc, poten_locs, alpha, beta)
        ...
        for i in range(0, np.shape(poten_locs)[0]):
            d2 = calc_dmap(poten_locs[i], poten_locs, alpha, beta)
...
def calc_dmap(curr_loc, poten_locs, alpha, beta):
    d_map = []
    for loc in poten_locs:
        loc_deg = (7.5 / 512) * np.linalg.norm(np.array(curr_loc) - np.array(loc))
